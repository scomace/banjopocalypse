...
import { CMD_HOOK, CMD_JUMP, CMD_RIGHT } from "../src/game/core/input";
...
run(CMD_HOOK, 1, "cast");
run(CMD_HOOK, 3, "flying");
run(CMD_HOOK, 20, "swinging+reel");
run(CMD_HOOK | CMD_RIGHT, 30, "pumping right");
run(CMD_HOOK | CMD_JUMP, 1, "jump-release");
run(0, 10, "airborne");
run(0, 60, "landed?");
// Cast straight into an enemy sitting on the 50-degree ray.
const p = sim.players[0];
const e = sim.enemies[0];
p.x = 480; p.y = 512; p.vx = 0; p.vy = 0; p.facing = 1; p.hook = null; p.hookCooldown = 0; p.grounded = true;
...
run(CMD_HOOK, 1, "cast at enemy");
run(CMD_HOOK, 4, "hook flying");
...
p.x = 480; p.y = 512; p.vx = 0; p.vy = 0; p.facing = 1; p.hook = null; p.hookCooldown = 0;
e2.x = 560; e2.y = 512; e2.vx = 0; e2.vy = 0; e2.phase = { kind: "normal" }; e2.flung = 0;
const hp2 = e2.hp;
run(CMD_HOOK | CMD_RIGHT, 1, "cast");
run(CMD_HOOK | CMD_RIGHT, 45, "swing right");
console.log("enemy1", e2.kind, "hp", hp2, "->", e2.hp, "flung", e2.flung, "phase", e2.phase.kind, "player alive", p.alive, "lives", p.livesLeft);
// Under-the-porch case: stand beneath the low left platform and cast.
p.x = 127; p.y = 512; p.vx = 0; p.vy = 0; p.facing = 1; p.hook = null; p.hookCooldown = 0; p.grounded = true; p.hookKick = 0;
run(0, 2, "porch setup");
run(CMD_HOOK, 1, "porch cast");
run(CMD_HOOK, 8, "porch fly");
run(CMD_HOOK, 50, "porch hold");
run(CMD_HOOK, 40, "porch hold more");
