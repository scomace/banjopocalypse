// Level geometry audit, as a pure module. Answers one question for a level:
// can the player actually GET there? Builds the graph of standing surfaces,
// derives the jump arc from the real sim constants, and BFSes from the P1
// spawn. Used by scripts/level-audit.mts (all 99 levels, plus a live-sim
// drift check) and by the level editor's lint panel (one level, live).

import { CAST, castJumpMult } from "../cast";
import { GRID_H, GRID_W, P_GRAVITY, P_JUMP_VY, P_MAX_FALL, P_MAX_SPEED, TILE } from "../sim/constants";
import { parseLevel } from "./parse";
import { T_EMPTY, T_SOLID, type LevelDef, type ParsedLevel } from "./types";
import { isBossLevel } from "./worlds";
...
export const WEAKEST_MULT = Math.min(...CAST.map((c) => castJumpMult(c.jump)));
export const ARC = arcRise(P_JUMP_VY * WEAKEST_MULT);
export const MAX_RISE = Math.max(...ARC);

/** Horizontal px available while the arc is at or above `need` px of rise. */
export function reachAt(need: number): number {
  let last = -1;
...
export const SAFETY = 6; // px of headroom demanded over the bare-minimum arc

/** Horizontal px you can cover while falling `rows` tiles from a standstill. */
export function fallReach(rows: number): number {
  let v = 0;
  let h = 0;
  let t = 0;
...
export function buildRuns(col: Uint8Array[]): Run[] {
  const runs: Run[] = [];
...
export function colGap(a: Run, b: Run): number {
...
export function runUnder(runs: Run[], px: number, py: number): Run | null {
...
export type AuditReport = {
  ...
  totalRuns: number;
  runs: Run[];
  /** ids of runs the BFS from the P1 spawn reached */
  reached: Set<number>;
  orphanRuns: Run[];
 ...
 */
function contentProblems(level: number, def: LevelDef, p: ParsedLevel): string[] {
  const out: string[] = [];
...
/** `def` must already carry the world's default enemies merged in. */
export function auditLevelDef(level: number, def: LevelDef): AuditReport {
  ...
  const isWallTexture = (r: Run) => r.c1 - r.c0 + 1 <= 2 && (r.c0 <= 1 || r.c1 >= GRID_W - 2);
